...
if (isset($_GET['id']) && intval($_GET['id'])) {
	...
	$titulo = 'CONSTANCIA OFICIAL DE NOTAS PARCIALES';
	...
	if ($result->rowcount()) {
		...
		<thead style="background:url(../librerias/barra.png) !important;">
		<tr>
		<th rowspan="2">N°<br><br></th>

		<th rowspan="2">MATERIA<br><br></th>
		...
		if ($result->rowcount()) {
			...
			foreach ($rows as $row) {
				...
				$html .= '<td>' . number_format($row['not_p1_act1'], 1) . '</td>';
				$html .= '<td>' . number_format($row['not_p1_act2'], 1) . '</td>';
				$html .= '<td>' . number_format($row['not_p1_act3'], 1) . '</td>';
				$html .= '<td style="background:#f2f2f2 !important;">' . number_format($row['not_p1_promuno'], 1) . '</td>';

				$html .= '<td>' . number_format($row['not_p2_act1'], 1) . '</td>';
				$html .= '<td>' . number_format($row['not_p2_act2'], 1) . '</td>';
				$html .= '<td>' . number_format($row['not_p2_act3'], 1) . '</td>';
				$html .= '<td style="background:#f2f2f2 !important;">' . number_format($row['not_p2_prom2'], 1) . '</td>';

				$html .= '<td>' . number_format($row['not_p3_act1'], 1) . '</td>';
				$html .= '<td>' . number_format($row['not_p3_act2'], 1) . '</td>';
				$html .= '<td>' . number_format($row['not_p3_act3'], 1) . '</td>';
				$html .= '<td style="background:#f2f2f2 !important;">' . number_format($row['not_p3_prom3'], 1) . '</td>';

				$html .= '<td style="background:#fff !important;">' . number_format($row['nota_prom_final'], 1) . '</td>';
				$html .= '</tr>';
				$i++;
			}
		} else {
		...
		$dompdf->render();
		$dompdf->stream("Notas Parciales de - " . $estu['est_nie'] . ".pdf", array("Attachment" => 0));
	} else {
